...
PRをレビューする際は、以下の観点を重視してください：

### 1. セキュリティチェック（最重要）
- 機密情報（APIキー、パスワード、トークン）の漏洩がないか
- XSS（クロスサイトスクリプティング）の脆弱性がないか
- ユーザー入力のバリデーションが適切か
- ファイルアップロード時の検証が適切か（ファイルタイプ、サイズ制限）
- 外部URLやリンクの安全性チェック
- `eval()` や `innerHTML` など危険な関数の使用がないか

### 2. コード品質
- TypeScriptの型安全性が保たれているか
- `any`型の使用を避けているか
- Vue 3のベストプラクティスに準拠しているか
- Composition API (`<script setup>`) を使用しているか
- 命名規則が一貫しているか

### 3. プロジェクト固有のルール
- すべてのコンポーネントは Composition API を使用すること
- モックデータは `src/data/` ディレクトリに配置すること
- 日本語のコメントとエラーメッセージを使用すること